...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(50, 10, utf8_decode('FECHA DE FINALIZACION'), 1, 0, 'C', 1);
        $this->Cell(45, 10, utf8_decode('DESCRIPCIÓN'), 1, 0, 'C', 1);
        $this->Cell(35, 10, utf8_decode('RESPONSABLE'), 1, 0, 'C', 1);
        $this->Cell(40, 10, utf8_decode('LINEA QUE APLICA'), 1, 1, 'C', 1);
    }
...
$consulta = "SELECT id_proyecto_investigacion, nombre, fecha_inicio, fecha_finalizacion,
            descripcion, responsable, linea_aplica FROM proyecto_investigacion
            WHERE nombre_cuerpo='$nombre_cuerpo' ";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $linea = utf8_decode($row['linea_aplica']);

    $pdf->Row([$id, $nombre, $fecha_formateada1, $fecha_formateada2, $descripcion, $responsable, $linea]);
}
