...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $this->Cell(100, 10, utf8_decode("REPORTE DE PROYECTOS DE INVESTIGACIÓN DEL CUERPO ACADÉMICO:"), 0, 1, 'C', 0);
        $this->SetTextColor(228, 100, 0);
        $this->Cell(88); // mover a la derecha
        $this->SetFont('Arial', 'B', 18);
        ...
        $this->Cell(50, 10, utf8_decode('RESPONSABLE'), 1, 0, 'C', 1);
        $this->Cell(35, 10, utf8_decode('FECHA DE INCIO'), 1, 0, 'C', 1);
        $this->Cell(55, 10, utf8_decode('FECHA DE FINALIZACION'), 1, 0, 'C', 1);
        $this->Cell(57, 10, utf8_decode('PRODUCCIÓN ACADÉMICA'), 1, 1, 'C', 1);
    }
...
$consulta = "SELECT proyecto_investigacion.id_proyecto_investigacion, proyecto_investigacion.nombre AS nombreproyecto, proyecto_investigacion.responsable, 
            proyecto_investigacion.fecha_inicio, proyecto_investigacion.fecha_finalizacion, producciones_academicas.nombre AS nombreproduccion
            FROM proyecto_investigacion
            INNER JOIN producciones_academicas
            ON producciones_academicas.proyec_inv_gen=proyecto_investigacion.nombre
            WHERE proyecto_investigacion.fecha_inicio LIKE '$fecha-%-%'
            AND proyecto_investigacion.nombre_cuerpo='$nombre_cuerpo'
            ORDER BY proyecto_investigacion.nombre";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $fecha_formateada1 = $datetime->format('d/m/Y');

    $fecha_finalizacion = $row['fecha_finalizacion'];
    $datetime = new DateTime($fecha_finalizacion);
    $fecha_formateada2 = $datetime->format('d/m/Y');

    $id = utf8_decode($row['id_proyecto_investigacion']);
    $nombreproyecto = utf8_decode($row['nombreproyecto']);
    $responsable = utf8_decode($row['responsable']);
    $nombreproduccion = utf8_decode($row['nombreproduccion']);

    $pdf->Row([$id, $nombreproyecto, $responsable,  $fecha_formateada1, $fecha_formateada2, $nombreproduccion]);
}

mysqli_close($con);

$pdf->Output('Reporte Proyectos de Investigacion Por Año.pdf', 'I'); //nombreDescarga, Visor(I->visualizar - D->descargar)
?>
